...
def copy_to_new_course():
    ...
    print('New course id is -> ' + asapcoursestocopy['COURSECODE'][i])
    logging.info('Starting copying process to new course id' + asapcoursestocopy['COURSECODE'][i])
    # Get new course
    new_course = canvas.get_course(asapcoursestocopy['COURSECODE'][i], use_sis_id=True)
    # Get old course
    ...
    user = canvas.get_user(asapcoursestocopy['EMAIL'][i],'sis_login_id')
    new_course.enroll_user(user.id, "TeacherEnrollment", enrollment={"notify": True,"enrollment_state": "active"})
...
if term_id == 0:
    ...
    for i in asapcoursestocopy.index:
        print(asapcoursestocopy['COURSECODE'][i])
        try:
            asapclass = canvas.get_course(asapcoursestocopy['COURSECODE'][i],use_sis_id=True)
            logging.info('Course already in Canvas.....now copying......')
            if configs['Debug'] == "True":
                dmsgbody = dmsgbody + asapcoursestocopy['COURSECODE'][i] + ' is in Canvas\n'
            copy_to_new_course()
        except CanvasException as e:
            if str(e) == "Not Found":
                #course does not exist, create it
                logging.info('Course ' + asapcoursestocopy['COURSECODE'][i] + ' is not in Canvas')
                print('Course not in Canvas, creating')
                dmsgbody = dmsgbody + asapcoursestocopy['COURSECODE'][i] + ' is NOT in Canvas\n'
                ...
                newCourse = subaccount.create_course(
                                    ...
                                    'course_code': asapcoursestocopy['COURSECODE'][i],
                                    'sis_course_id': asapcoursestocopy['COURSECODE'][i],
                                    'term_id': term_id}
                                    )
                print(newCourse)
                dmsgbody = dmsgbody + 'created course ' + asapcoursestocopy['COURSECODE'][i] + ' in Canvas\n'
                copy_to_new_course()
